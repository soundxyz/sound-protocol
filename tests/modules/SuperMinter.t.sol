...
import { ISuperMinter, SuperMinter } from "@modules/SuperMinter.sol";
...
contract SuperMinterTests is TestConfigV2 {
    SuperMinter sm;
    SoundEditionV2 edition;
    Merkle merkle;

    struct SuperMinterConstants {
    ...
    function _superMinterConstants() internal view returns (SuperMinterConstants memory smc) {
    ...
    function test_createMints() public {
        ...
        for (uint256 j; j < 3; ++j) {
            for (uint256 i; i < 3; ++i) {
                ISuperMinter.MintCreation memory c;
        ...
        ISuperMinter.MintInfo[] memory mintInfoList = sm.mintInfoList(address(edition));
        assertEq(mintInfoList.length, 3 * 3);
        for (uint256 j; j < 3; ++j) {
            for (uint256 i; i < 3; ++i) {
                ISuperMinter.MintInfo memory info = mintInfoList[j * 3 + i];
    ...
    function test_settersConfigurable(uint256) public {
        ...
        ISuperMinter.MintInfo memory info = sm.mintInfo(address(edition), c.tier, 0);
        assertEq(info.platform, address(this));
        if (c.tier == 0) {
            if (c.mode == sm.DEFAULT()) {
                assertEq(info.signer, address(0));
                vm.expectRevert(ISuperMinter.NotConfigurable.selector);
    ...
    function test_mintDefaultUpToMaxPerAccount() public {
        ...
        p.tier = 1;
        p.quantity = 9;
        vm.expectRevert(ISuperMinter.ExceedsMaxPerAccount.selector);
        sm.mintTo{ value: p.quantity * 1 ether }(p);
    ...
    function test_mintMerkleUpToMaxPerAccount() public {
        ...
        vm.expectRevert(ISuperMinter.InvalidMerkleProof.selector);
        sm.mintTo{ value: p.quantity * 1 ether }(p);
        // Restore the proof.
        p.allowlistProof[0] = bytes32(uint256(p.allowlistProof[0]) ^ 1);
    ...
    function test_mintMerkleWithDelegate() public {
        ...
        vm.expectRevert(ISuperMinter.CallerNotDelegated.selector);
        sm.mintTo{ value: p.quantity * 1 ether }(p);
    ...
    function test_platformFeeConfig(uint256) public {
        SuperMinterConstants memory smc = _superMinterConstants();
        ...
        if (expectRevert) vm.expectRevert(ISuperMinter.InvalidPlatformFeeConfig.selector);
    ...
    function _setDefaultPlatformFeeConfig(
        ...
        ISuperMinter.PlatformFeeConfig memory c;
    ...
    function _checkPlatformFeeConfig(
        ISuperMinter.PlatformFeeConfig memory result,
        uint96 perTxFlat,
        uint96 perMintFlat,
        uint16 perMintBPS,
        bool active
    ...
    function test_unitPrice(uint256) public {
        ...
        ISuperMinter.TotalPriceAndFees memory tpaf;
        if (c.mode == sm.VERIFY_SIGNATURE() && signedPrice < c.price) {
            vm.expectRevert(ISuperMinter.SignedPriceTooLow.selector);
    ...
    function test_mintWithVariousFees(uint256) public {
        ...
        ISuperMinter.MintInfo memory mintInfo = sm.mintInfo(address(edition), 1, 0);
        assertEq(mintInfo.price, c.price);
        assertEq(mintInfo.affiliateFeeBPS, c.affiliateFeeBPS);

        ISuperMinter.PlatformFeeConfig memory pfc;
        ...
        if (affiliated) {
            ...
            if (affiliateLeaves.length != 0) {
                ...
                if (_random() % 4 == 0) {
                    ...
                    vm.expectRevert(ISuperMinter.InvalidAffiliate.selector);
        ...
        if (affiliated) {
            ...
            vm.expectRevert(ISuperMinter.PlatformFeeAddressIsZero.selector);
    ...
    function _generateSignature(ISuperMinter.MintTo memory p, uint256 privateKey)
        internal
