...
const containsChangeset = danger.git.created_files.some((file) =>
  CHANGESET_REGEX_CHECK.test(file)
);

if (!containsChangeset) {
  const noChangesetNeeded = danger.github.issue.labels.some(
    (label) => label.name === NO_CHANGESET_LABEL
  );
